...
pub fn execute(ssh_client: &SshClient, args: &Value, use_become: bool, become_user: &str) -> Result<()> {
    let map = match args {
        Value::Mapping(map) => map,
        _ => return Err(anyhow::anyhow!("Package module requires a mapping of arguments")),
    };
    ...
    let packages = match map.get(&Value::String("name".to_string())) {
        ...
        Some(Value::Sequence(names)) => {
            ...
            if package_names.is_empty() {
                return Err(anyhow::anyhow!("Package module requires at least one package name"));
            }
            package_names
        },
        _ => return Err(anyhow::anyhow!("Package module requires a 'name' parameter (string or list)")),
    };
    ...
    let state = PackageState::from_str(state_str)
        .context("Failed to parse package state")?;
    ...
    if update_cache {
        ...
        if exit_code != 0 && exit_code != 100 {
            error!("Failed to update package cache with exit code: {}", exit_code);
            return Err(anyhow::anyhow!("Failed to update package cache with exit code: {}", exit_code));
        }
    }
    ...
    for package_name in packages {
        ...
        let command = match (pkg_manager.clone(), &state) {
            (PackageManager::Apt, PackageState::Present) => format!("apt-get -y install {}", package_name),
            (PackageManager::Apt, PackageState::Absent) => format!("apt-get -y remove {}", package_name),
            (PackageManager::Apt, PackageState::Latest) => format!("apt-get -y install --only-upgrade {}", package_name),
            
            (PackageManager::Yum, PackageState::Present) => format!("yum -y install {}", package_name),
            (PackageManager::Yum, PackageState::Absent) => format!("yum -y remove {}", package_name),
            (PackageManager::Yum, PackageState::Latest) => format!("yum -y update {}", package_name),
            
            (PackageManager::Dnf, PackageState::Present) => format!("dnf -y install {}", package_name),
            (PackageManager::Dnf, PackageState::Absent) => format!("dnf -y remove {}", package_name),
            (PackageManager::Dnf, PackageState::Latest) => format!("dnf -y update {}", package_name),
            
            (PackageManager::Zypper, PackageState::Present) => format!("zypper --non-interactive install {}", package_name),
            (PackageManager::Zypper, PackageState::Absent) => format!("zypper --non-interactive remove {}", package_name),
            (PackageManager::Zypper, PackageState::Latest) => format!("zypper --non-interactive update {}", package_name),
            
            (PackageManager::Pacman, PackageState::Present) => format!("pacman -S --noconfirm {}", package_name),
            (PackageManager::Pacman, PackageState::Absent) => format!("pacman -R --noconfirm {}", package_name),
            (PackageManager::Pacman, PackageState::Latest) => format!("pacman -Syu --noconfirm {}", package_name),
        };

        info!("Executing package command: {}", command);
        
        // Check if package is already in desired state
        if should_skip_operation(ssh_client, &package_name, &state, pkg_manager.clone(), use_become, become_user)? {
            info!("Package '{}' is already in desired state '{}', skipping", package_name, state_str);
            continue;
        }
        ...
        if exit_code != 0 {
            ...
            return Err(anyhow::anyhow!("Package command failed for '{}' with exit code: {}", package_name, exit_code));
        }
...
fn detect_package_manager(ssh_client: &SshClient, use_become: bool, become_user: &str) -> Result<PackageManager> {
...
fn should_skip_operation(
    ...
    let check_cmd = match pkg_manager {
        PackageManager::Apt => format!("dpkg-query -W -f='${{Status}}' {} 2>/dev/null | grep -q 'ok installed'", package_name),
        PackageManager::Yum | PackageManager::Dnf => format!("rpm -q {} >/dev/null 2>&1", package_name),
