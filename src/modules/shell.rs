...
impl ModuleExecutor for ShellModule {
    fn execute(ssh_client: &SshClient, shell_args: &Value, use_become: bool, become_user: &str) -> Result<ModuleResult> {
        ...
        let (exit_code, stdout, stderr) = Self::execute_command(ssh_client, &shell_wrapped, use_become, become_user)?;
