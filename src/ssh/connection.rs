...
impl SshConnection for SshClient {
    fn execute_command(&self, command: &str) -> Result<(i32, String, String)> {
        ...
        debug!(
            ...
            self.host, command_without_password
        );
        ...
        channel.exec(command).context(format!(
            "Failed to execute command: {}",
            command_without_password
        ))?;
    ...
    fn execute_sudo_command(
        ...
        let sudo_cmd = if sudo_user.is_empty() || sudo_user == "root" {
            ...
            format!(
                "echo \"{}\" | sudo -S -u {} {}",
                self.sudo_password, sudo_user, command
            )
        };
    ...
    fn write_file_with_sudo(
        ...
        debug!(
            "Writing content to {} via temporary file {}",
        ...
        let (original_mode, original_owner, original_group) = if file_exists {
            ...
            let (stat_exit_code, stat_output, stat_stderr) =
                self.execute_sudo_command(&stat_cmd, "")?;

            if stat_exit_code == 0 {
                let parts: Vec<&str> = stat_output.trim().split_whitespace().collect();
                if parts.len() >= 3 {
                    debug!(
                        "Original file permissions: mode={}, owner={}, group={}",
        ...
        if target_owner.is_some() || target_group.is_some() {
            ...
            if !ownership.is_empty() {
                ...
                let (chown_exit_code, _, chown_stderr) =
                    self.execute_sudo_command(&chown_cmd, "")?;
        ...
        info!(
            "Successfully wrote file {} with sudo privileges",
            remote_path
        );
        Ok(())
    }
    ...
    fn upload_file(&self, local_path: &str, remote_path: &str) -> Result<()> {
        ...
        remote_file
            .send_eof()
            .context("Failed to finalize SCP upload")?;
        remote_file
            .wait_eof()
            .context("Failed to wait for SCP EOF confirmation")?;
...
impl SshClient {
    pub fn connect(host: &Host) -> Result<Self> {
        ...
        let tcp = TcpStream::connect(format!("{}:{}", host.hostname, host.port)).context(
            format!("Failed to connect to {}:{}", host.hostname, host.port),
        )?;
        ...
        // 获取用户名
        let username = host.get_ssh_user().map(|s| s.as_str()).unwrap_or("root");
        // 获取sudo密码
        ...
        if !auth_succeeded {
            if let Some(key_path) = host.get_ssh_private_key() {
                ...
                debug!(
                    "Attempting private key authentication with key: {}",
                    key_path
                );
    ...
    pub fn upload_sudo_file(&self, local_path: &str, remote_path: &str) -> Result<()> {
        ...
        self.write_file_with_sudo(content.as_str(), remote_path, None, None, None)
    }

    // Forwarding methods to trait implementation for backward compatibility
...
mod tests {
    ...
    fn test_ssh_client_struct() {
        // We can't easily test connect() without a real server, but we can check the struct layout
        // and ensure it implements the trait.
        // This test mainly verifies that SshClient implements SshConnection trait
    ...
    fn test_connect_invalid_host() {
        // Test that connecting to an invalid host returns an error
        ...
        // This should fail with DNS or connection error
