...
pub fn parse_inventory(inventory_path: &str) -> Result<Inventory> {
    ...
    for (line_num, line_result) in reader.lines().enumerate() {
        let line = line_result.context(format!(
            "Failed to read line {} from inventory file",
            line_num + 1
        ))?;
        let line = line.trim();
        ...
        if let Some(group_ref) = &current_group {
            if group_ref.ends_with(":vars") {
                if let Some(caps) = var_line_re.captures(line) {
                    ...
                    debug!(
                        "Setting group variable for {}: {} = {}",
                        group_name, var_name, var_value
                    );
        ...
        if let Some(caps) = host_port_re.captures(host_entry) {
            ...
            debug!(
                "Host with port specification: {} (port: {})",
                host_name, port
            );
        }
        ...
        if !inventory.hosts.contains_key(host_name) {
            ...
            let host = Host::new(host_name).with_port(port);
...
ansible_ssh_pass=testpassword
        ...
        // 验证 all:vars 被继承
        assert_eq!(
            test1.get_variable("ansible_ssh_pass").unwrap(),
            "testpassword"
        );
