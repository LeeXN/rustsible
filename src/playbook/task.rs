...
impl Task {
    ...
    fn execute_module(&self, host: &Host, tera: &mut Tera, _context: &TeraContext, vars: &HashMap<String, Value>) -> Result<TaskResult> {
        ...
        if is_local {
            ...
            let module_result = match self.module.as_str() {
                ...
                "template" => {
                    ...
                    if let Some(Value::Mapping(existing_vars)) = template_args.get(&Value::String("vars".to_string())) {
                    ...
                    template_args.insert(Value::String("vars".to_string()), Value::Mapping(vars_mapping));
        ...
        let module_result = match self.module.as_str() {
            "command" | "shell" => {
                debug!("Executing command/shell module with args: {:?}", resolved_args);
                let raw_param_value = resolved_args.get(&Value::String("_raw_params".to_string())).cloned().unwrap_or(Value::Null);
                ...
                crate::modules::command::execute(&client, &Value::String(command), self.is_become, &self.become_user)?
            },
            "debug" => {
            ...
            "copy" => {
                ...
                crate::modules::copy::execute(&client, &Value::Mapping(resolved_args), self.is_become, &self.become_user)?
            },
            "file" => {
                debug!("Executing file module with args: {:?}", resolved_args);
                crate::modules::file::execute(&client, &Value::Mapping(resolved_args), self.is_become, &self.become_user)?
            },
            "template" => {
                ...
                debug!("Enhanced template args with task variables: {:?}", template_args);
                crate::modules::template::execute(&client, &Value::Mapping(template_args), self.is_become, &self.become_user)?
            },
            "package" => {
                debug!("Executing package module with args: {:?}", resolved_args);
                crate::modules::package::execute(&client, &Value::Mapping(resolved_args), self.is_become, &self.become_user)?
            },
            "service" => {
                debug!("Executing service module with args: {:?}", resolved_args);
                crate::modules::service::execute(&client, &Value::Mapping(resolved_args), self.is_become, &self.become_user)?
            },
            "lineinfile" => {
                crate::modules::lineinfile::execute(&client, &Value::Mapping(resolved_args), self.is_become, &self.become_user)?
            },
            "user" => {
                crate::modules::user::execute(&client, &Value::Mapping(resolved_args), self.is_become, &self.become_user)?
            },
            _ => {
                let mut result = ModuleResult::default();
    ...
    fn resolve_args(
        ...
        if let Err(e) = arg_tera.add_raw_template("tmp_if_expr", "{% if true %}true{% else %}false{% endif %}") {
                ...
                if value.starts_with("{{") && value.ends_with("}}") {
                    ...
                    if let Some(if_pos) = inner.find(" if ") {
                        if let Some(else_pos) = inner.find(" else ") {
                            ...
                            debug!("预处理 if 条件: then={}, condition={}, else={}", then_expr, condition, else_expr);
                            return format!("{{% if {} %}}{}{{% else %}}{}{{% endif %}}", 
        ...
        for (key, value) in &self.args {
            match value {
                Value::String(s) => {
                    ...
                    let is_mode_param = (self.module == "file" || self.module == "copy" || self.module == "template") && 
                                       (key == "mode" || key == "mode");
                    
                    // 预处理条件表达式
                    let processed_value = if is_mode_param && (s.contains(" if ") || s.contains(" else ")) {
                    ...
                    match crate::playbook::templar::render_value(&processed_value, &mut arg_tera, context, force_string) {
                        Ok(rendered_value) => {
                            debug!(
                            ...
                            if is_mode_param {
                                if let Value::String(mode_str) = &rendered_value {
                                    if mode_str.contains("{{") || mode_str.contains("{%") {
                                        ...
                                        resolved.insert(key.clone(), Value::String("0644".to_string()));
                                        continue;
                                    }
                                }
                            }
                        ...
                        Err(e) => {
                            ...
                            let is_critical_param = match (self.module.as_str(), key) {
                                ...
                                ("template", k) if k == &Value::String("content".to_string()) => true,
                                _ => false,
                            };
