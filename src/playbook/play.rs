...
impl Play {
    pub fn execute(&self, hosts: &[Host]) -> Result<()> {
        ...
        // Prepare play variables
        let mut play_vars: HashMap<String, Value> = HashMap::new();
        
        // Convert mapping to HashMap
        for (key, value) in &self.vars {
            if let Value::String(k) = key {
                play_vars.insert(k.clone(), value.clone());
            }
        }
        ...
        for (task_index, task) in self.tasks.iter().enumerate() {
            ...
            for host in hosts {
                // Execute the task on this host
                match effective_task.execute(host, &play_vars) {
                    Ok(result) => {
                        ...
                        if let Some(register_var) = &effective_task.register {
                            ...
                            // Store in play variables
                            play_vars.insert(register_var.clone(), Value::Mapping(result_value));
                        }
        ...
        if !notified_handlers.is_empty() {
            ...
            for handler in &self.handlers {
                ...
                if notified_handlers.contains(handler_name) {
                    ...
                    for host in hosts {
                        if let Err(e) = effective_handler.execute(host, &play_vars) {
